        ...
        while True:
            ...
            try:
                if choice == "1":
                    ...
                    key = input(
                        "Введите имя поля для фильтра (например, first_name): "
                    ).strip()
                    ...
                    if new_last:
                        update_data["second_name"] = new_last
                    if new_age:
                        update_data["age"] = int(new_age)
                    if new_sex:
                        update_data["sex"] = new_sex

                    if update_data:
                        self.database.update_records(
                            self.table_name, update_data, student_id=student_id
                        )
